...
const { getUserContextForEntityType } = require('./sdm_context');
...
// Versión asíncrona con contexto dinámico
async function formatEntityWithDynamicContext(json, isJSONLD, transformFlags = {}) {
    const version = `${transformFlags.version || Config.getConfig().version}.1`;
    const picks = transformFlags.pick ? transformFlags.pick.split(',') : [];
    const omits = transformFlags.omit ? transformFlags.omit.split(',') : [];
    ...
    const obj = {};
    if (isJSONLD) {
        const userContext = await getUserContextForEntityType(json.type);
        obj['@context'] = [userContext, Config.getConfig().coreContext];
    }
...
exports.formatEntityWithDynamicContext = formatEntityWithDynamicContext;
