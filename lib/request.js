...
function v2BrokerURL(url) {
    const base = Config.getConfig().v2ContextBroker.replace(/\/+$/, '');
    const pathPart = url.replace(/^\/+/, '');
    return `${base}/v2/${pathPart}`;
}
...
async function sendRequest(path, options) {
    ...
    const NGSI_V2 = require('./ngsi-v2');
    const res = options.res;
    let extraHeaders = {};
    let localsHeaders = {};
    if (res) {
        extraHeaders = NGSI_V2.setHeaders(res);
        if (res.locals && res.locals.headers) {
            localsHeaders = res.locals.headers;
        }
        delete options.res;
    }
    options.headers = { ...(options.headers || {}), ...localsHeaders, ...extraHeaders };
    // Get domain and project from headers
    const domain = options.headers['fiware-service'];
    const project = options.headers['fiware-servicepath'];
    options.headers['X-Auth-Token'] = await auth.getToken(domain, project);
    // Detailed log of the request
    console.log('--- Lepus -> NGSI-v2 REQUEST ---');
    console.log('URL:', v2BrokerURL(path));
